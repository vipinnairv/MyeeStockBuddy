...
const csPush = _csDebounce(async function () {
  ...
  } catch (e) { try { _logErr(e, 'cloudSync:push'); } catch (_) {} }
}, 2500);
...
async function csPull() {
  ...
  catch (e) { try { _logErr(e, 'cloudSync:pull'); } catch (_) {} return null; }
}
  ...
  if (user && !user.emailVerified) {
    gate.innerHTML = `<div class="pm-auth-card">
      ...
      <div class="pm-auth-sub">A verification link was sent to <b>${(user.email || '').replace(/</g,'&lt;')}</b>.
        Click it, then reload this page.</div>
      <button class="btn btn-pri" onclick="pmAuthResendVerification()">Resend verification email</button>
